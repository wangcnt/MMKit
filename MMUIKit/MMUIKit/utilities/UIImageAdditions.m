...
- (NSData *)compressToDataWithMaxSize:(NSInteger)maxSize {
    ...
    while (data.length > maxSize && compress > 0.01) {
        compress -= 0.02f;
        data = UIImageJPEGRepresentation(self, compress);
    }
...
- (UIImage *)imageByCompressingToMaxSize:(NSInteger)maxSize {
    NSData *data = [self compressToDataWithMaxSize:maxSize];
...
- (UIImage *)imageByScalingToFitSize:(CGSize)size
{
    CGFloat aspect = self.size.width / self.size.height;
    if (size.width / aspect <= size.height) {
        return [self imageByScalingToSize:CGSizeMake(size.width, size.width / aspect)];
    } else {
        return [self imageByScalingToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)blur
{
    return [self blurredImageWithLightAlpha:0.1 radius:3 colorSaturationFactor:1];
}

- (UIImage *)blurredImageWithLightAlpha:(CGFloat)alpha radius:(CGFloat)radius colorSaturationFactor:(CGFloat)colorSaturationFactor
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:alpha];
    return [self blurredImageWithRadius:radius tintColor:tintColor saturationDeltaFactor:colorSaturationFactor maskImage:nil];
}
