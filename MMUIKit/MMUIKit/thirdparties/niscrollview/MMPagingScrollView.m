...
#import "MMPagingScrollViewCell.h"
#import "MMPagingScrollViewCell+Subclassing.h"
...
const NSInteger MMPagingScrollViewUnknownNumberOfPages = -1;
const CGFloat MMPagingScrollViewDefaultPageMargin = 10;
const CGFloat MMPagingScrollViewDefaultPageInset = 0;

@implementation MMPagingScrollView {
    MMViewRecycler* _viewRecycler;
...
- (void)commonInit {
    // Default state.
    self.pageMargin = MMPagingScrollViewDefaultPageMargin;
    self.pageInset = MMPagingScrollViewDefaultPageInset;
    self.type = MMPagingScrollViewHorizontal;
    ...
    _centerPageIndex = -1;
    _numberOfPages = MMPagingScrollViewUnknownNumberOfPages;
    
    _viewRecycler = [[MMViewRecycler alloc] init];
...
- (CGRect)frameForPagingScrollView {
    ...
    if (MMPagingScrollViewHorizontal == self.type) {
    ...
    } else if (MMPagingScrollViewVertical == self.type) {
...
- (BOOL)isDisplayingPageForIndex:(NSInteger)pageIndex {
    ...
    for (UIView <MMPagingScrollViewPage>* page in _visiblePages) {
...
- (NSInteger)currentVisiblePageIndex {
    ...
    if (MMPagingScrollViewHorizontal == self.type) {
        ...
        // visible image.
        return  mm_bound_integer((NSInteger)(mm_floor_float((contentOffset.x + boundsSize.width / 2) / boundsSize.width)
                                    + 0.5f),
                        0, self.numberOfPages - 1);
        
    } else if (MMPagingScrollViewVertical == self.type) {
        return mm_bound_integer((NSInteger)(mm_floor_float((contentOffset.y + boundsSize.height / 2) / boundsSize.height)
                                    + 0.5f),
                        0, self.numberOfPages - 1);
    }
...
- (NSRange)rangeOfVisiblePages {
    ...
    NSInteger firstVisiblePageIndex = mm_bound_integer(currentVisiblePageIndex - visibleRange, 0, self.numberOfPages - 1);
    NSInteger lastVisiblePageIndex  = mm_bound_integer(currentVisiblePageIndex + visibleRange, 0, self.numberOfPages - 1);
    
    return NSMakeRange(firstVisiblePageIndex, lastVisiblePageIndex - firstVisiblePageIndex + 1);
}

- (void)willDisplayPage:(UIView<MMPagingScrollViewPage> *)pageView atIndex:(NSInteger)pageIndex {
...
- (void)resetPage:(id<MMPagingScrollViewPage>)page {
...
- (void)resetSurroundingPages {
    for (id<MMPagingScrollViewPage> page in _visiblePages) {
...
- (UIView<MMPagingScrollViewPage> *)dequeueReusablePageWithIdentifier:(NSString *)identifier {
    ...
    return (UIView<MMPagingScrollViewPage> *)[_viewRecycler dequeueReusableViewWithIdentifier:identifier];
}

- (UIView<MMPagingScrollViewPage> *)loadPageAtIndex:(NSInteger)pageIndex {
    UIView<MMPagingScrollViewPage>* page = [self.dataSource pagingScrollView:self pageViewForIndex:pageIndex];
    if (nil == page || ![page isKindOfClass:[UIView class]]
        || ![page conformsToProtocol:@protocol(MMPagingScrollViewPage)]) {
...
- (void)displayPageAtIndex:(NSInteger)pageIndex {
    UIView<MMPagingScrollViewPage>* page = [self loadPageAtIndex:pageIndex];
    if (nil == page) {
        return;
    }
...
- (void)recyclePageAtIndex:(NSInteger)pageIndex {
    for (UIView<MMPagingScrollViewPage>* page in [_visiblePages copy]) {
        if (page.pageIndex == pageIndex) {
...
- (void)reloadData {
    ...
    if (oldCenterPageIndex >= 0) {
        _centerPageIndex = mm_bound_integer(_centerPageIndex, 0, self.numberOfPages - 1);
...
- (void)willRotateToInterfaceOrientation: (UIInterfaceOrientation)toInterfaceOrientation
    ...
    if (offset >= 0) {
        _firstVisiblePageIndexBeforeRotation = (NSInteger)mm_floor_float(offset / pageScrollableDimension);
...
- (void)setType:(MMPagingScrollViewType)type {
    if (_type != type) {
        _type = type;
        _scrollView.scrollsToTop = (type == MMPagingScrollViewVertical);
    }
}
