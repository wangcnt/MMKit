...
+ (MMKeychain *)keychainWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (MMKeychain *)keychainWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
+ (MMKeychain *)keychainWithServer:(NSURL *)server protocolType:(MMKeychainProtocolType)protocolType
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:MMKeychainAuthenticationTypeDefault];
}

+ (MMKeychain *)keychainWithServer:(NSURL *)server protocolType:(MMKeychainProtocolType)protocolType authenticationType:(MMKeychainAuthenticationType)authenticationType
{
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = MMKeychainItemClassGenericPassword;
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(MMKeychainProtocolType)protocolType
{
    return [self initWithServer:server protocolType:protocolType authenticationType:MMKeychainAuthenticationTypeDefault];
}

- (instancetype)initWithServer:(NSURL *)server protocolType:(MMKeychainProtocolType)protocolType authenticationType:(MMKeychainAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = MMKeychainItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = MMKeychainAccessibilityAfterFirstUnlock;
}
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    MMKeychain *keychain = [MMKeychain keychainWithService:service accessGroup:accessGroup];
...
+ (NSArray MM_KEY_TYPE *)allKeysWithItemClass:(MMKeychainItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == MMKeychainItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == MMKeychainItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(MMKeychainItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == MMKeychainItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (void)setAccessibility:(MMKeychainAccessibility)accessibility authenticationPolicy:(MMKeychainAuthenticationPolicy)authenticationPolicy
{
...
- (void)sharedPasswordWithCompletion:(void (^)(NSString *account, NSString *password, NSError *error))completion
{
    ...
    if (domain.length > 0) {
        ...
        NSError *error = [self.class argumentError:NSLocalizedString(@"the server property must not to be nil, should use 'keychainWithServer:protocolType:' initializer to instantiate keychain store", nil)];
        if (completion) {
            completion(nil, nil, error);
        }
    }
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray MM_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
    [self requestSharedWebCredentialForDomain:nil account:nil completion:completion];
}

+ (void)requestSharedWebCredentialForDomain:(NSString *)domain account:(NSString *)account completion:(void (^)(NSArray MM_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == MMKeychainAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'MMKeychainAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'MMKeychainAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case MMKeychainItemClassGenericPassword:
            return kSecClassGenericPassword;
        case MMKeychainItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case MMKeychainProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case MMKeychainProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case MMKeychainProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case MMKeychainProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case MMKeychainProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case MMKeychainProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case MMKeychainProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case MMKeychainProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case MMKeychainProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case MMKeychainProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case MMKeychainProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case MMKeychainProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case MMKeychainProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case MMKeychainProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case MMKeychainProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case MMKeychainProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case MMKeychainProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case MMKeychainProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case MMKeychainProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case MMKeychainProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case MMKeychainProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case MMKeychainProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case MMKeychainProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case MMKeychainProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case MMKeychainProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case MMKeychainProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case MMKeychainProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case MMKeychainProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case MMKeychainProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case MMKeychainAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case MMKeychainAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case MMKeychainAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case MMKeychainAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case MMKeychainAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case MMKeychainAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case MMKeychainAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case MMKeychainAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case MMKeychainAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case MMKeychainAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case MMKeychainAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case MMKeychainAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case MMKeychainAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case MMKeychainAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case MMKeychainAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:MMKeychainErrorDomain code:MMKeychainErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:MMKeychainErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:MMKeychainErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:MMKeychainErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation MMKeychain (Deprecation)
