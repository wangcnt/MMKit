...
@interface MMMediator ()

@property (nonatomic, strong) NSMutableDictionary *cachedTarget;

@end

@implementation MMMediator

#pragma mark - public methods
+ (instancetype)sharedInstance
{
    static MMMediator *mediator;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mediator = [[MMMediator alloc] init];
    });
    return mediator;
}
...
- (id)resourceForURL:(NSURL *)url withCompletion:(void(^)(NSDictionary *info))completion {
    ...
    for (NSString *param in [urlString componentsSeparatedByString:@"&"]) {
        ...
        parameters[elts.firstObject] = elts.lastObject;
    }
    ...
    id result = [self performAction:actionName withTarget:url.host parameters:parameters shouldCacheTarget:NO];
    if (completion) {
        if (result) {
