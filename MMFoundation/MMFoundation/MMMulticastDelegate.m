...
@interface MMMulticastDelegateNode : NSObject {
@private
...
@interface MMMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation MMMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	MMMulticastDelegateNode *node =
	    [[MMMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	{
		MMMulticastDelegateNode *node = delegateNodes[i - 1];
		...
		if (delegate == nodeDelegate)
		{
			...
			{
				// Recall that this node may be retained by a MMMulticastDelegateEnumerator.
...
{
	for (MMMulticastDelegateNode *node in delegateNodes)
	{
		node.delegate = nil;
		#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
		node.unsafeDelegate = nil;
...
- (MMMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[MMMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation MMMulticastDelegateNode
...
@implementation MMMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		MMMulticastDelegateNode *node = delegateNodes[currentNodeIndex];
		currentNodeIndex++;
