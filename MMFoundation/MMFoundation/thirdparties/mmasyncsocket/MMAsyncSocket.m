...
@interface MMAsyncSocket (Private)
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create("MMAsyncSocket", NULL);
		}
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{
			...
			MMAsyncSocket *acceptedSocket = [[MMAsyncSocket alloc] initWithDelegate:delegate
...
- (void)doWriteData
{
	...
	// Note: This method is not called if theCurrentWrite is an MMAsyncSpecialPacket (startTLS packet)
...
{
	MMAsyncSocket *asyncSocket = (MMAsyncSocket *)connection;
...
+ (void)addStreamListener:(MMAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(MMAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	MMAsyncSocket *asyncSocket = [(MMAsyncSocket *)pInfo retain];
